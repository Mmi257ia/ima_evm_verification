...
static __always_inline long bpf_get_path(char *kpath, const char *upath)
{
	const size_t size = PATH_SIZE;

	long ret = bpf_probe_read_user_str(kpath, size, upath);
	if (ret < 0) {
		bpf_printk("bpf_get_path failed %ld for reading from %px", ret,
			   upath);
...
static __always_inline long bpf_get_xattr_name(char *xname, const char *uname)
{
...
static __always_inline long bpf_get_xattr_value(u8 *to, size_t size,
						void *ufrom)
{
...
static __always_inline const struct file *
...
static __always_inline const struct inode *
