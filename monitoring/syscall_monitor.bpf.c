...
int BPF_KRETPROBE(handle_evm_calc_hmac_or_hash_ret)
{
	...
	if (data.size > HASH_MAX_DIGESTSIZE) {
		bpf_printk("evm_calc_hmac_or_hash_ret: digest size (%llu) "
			   "is more that the maximum one (%d)",
			   data.size, HASH_MAX_DIGESTSIZE);
		goto CLEANUP;
	}
...
static __always_inline struct event *read_main_args()
{
	if (!should_monitor()) {
		return 0;
	}
...
save_syscall_args(struct trace_event_raw_sys_enter *ctx)
{
	...
	long ret;
	if ((ret = bpf_map_update_elem(&args_map, &pid_tgid, &args, BPF_ANY)) <
	    0) {
...
int trace_exit_open(struct trace_event_raw_sys_exit *ctx)
{
	...
	bpf_get_path(e->syscall.open.pathname,
		     (const char *)e->syscall.args[0]);
...
int trace_exit_openat(struct trace_event_raw_sys_exit *ctx)
{
	...
	bpf_get_path(e->syscall.openat.pathname,
		     (const char *)e->syscall.args[1]);
...
int trace_exit_creat(struct trace_event_raw_sys_exit *ctx)
{
	...
	bpf_get_path(e->syscall.creat.pathname,
		     (const char *)e->syscall.args[0]);
...
int BPF_KPROBE(handle_vfs_mkdir, struct user_namespace *mnt_userns,
...
int BPF_KRETPROBE(handle_vfs_mkdir_ret)
{
	...
	u64 id = bpf_get_current_pid_tgid();
	struct mkdir_dentry_data *dentryp =
		bpf_map_lookup_elem(&mkdir_dentry, &id);
	if (!dentryp) {
...
int trace_exit_chmod(struct trace_event_raw_sys_exit *ctx)
{
	...
	__builtin_memcpy(&e->syscall.chmod.evm_hash, evm_data,
			 sizeof(e->syscall.chmod.evm_hash));
...
int trace_exit_fchmod(struct trace_event_raw_sys_exit *ctx)
{
	...
	__builtin_memcpy(&e->syscall.fchmod.evm_hash, evm_data,
			 sizeof(e->syscall.fchmod.evm_hash));
...
int trace_exit_chown(struct trace_event_raw_sys_exit *ctx)
{
	...
	__builtin_memcpy(&e->syscall.chown.evm_hash, evm_data,
			 sizeof(e->syscall.chown.evm_hash));
...
int trace_exit_fchown(struct trace_event_raw_sys_exit *ctx)
{
	...
	__builtin_memcpy(&e->syscall.fchown.evm_hash, evm_data,
			 sizeof(e->syscall.fchown.evm_hash));
...
int trace_exit_getxattr(struct trace_event_raw_sys_exit *ctx)
{
	...
	bpf_get_xattr_name(e->syscall.getxattr.name,
			   (char *)e->syscall.args[1]);
	e->syscall.getxattr.addr = (void *)e->syscall.args[2];
	e->syscall.getxattr.size = e->syscall.args[3];
	bpf_get_xattr_value(e->syscall.getxattr.value, e->syscall.getxattr.size,
			    e->syscall.getxattr.addr);
...
int trace_exit_setxattr(struct trace_event_raw_sys_exit *ctx)
{
	...
	bpf_get_xattr_name(e->syscall.setxattr.name,
			   (char *)e->syscall.args[1]);
	e->syscall.setxattr.size = e->syscall.args[3];
	e->syscall.setxattr.flags = e->syscall.args[4];
	bpf_get_xattr_value(e->syscall.setxattr.value, e->syscall.setxattr.size,
			    (uint8_t *)e->syscall.args[2]);
...
int trace_exit_execve(struct trace_event_raw_sys_exit *ctx)
{
	...
	if (e->syscall.ret < 0) {
		bpf_get_path(e->syscall.execve.pathname,
			     (char *)e->syscall.args[0]);
	} else {
		...
		if (!(data = bpf_map_lookup_elem(&execve_map, &pid_tgid))) {
			...
			__builtin_memcpy(e->syscall.execve.pathname,
...
int trace_sched_process_exec(struct trace_event_raw_sched_process_exec *ctx)
{
	...
	struct execve_data data = {};
	bpf_probe_read_kernel_str(data.pathname, sizeof data.pathname,
				  filename);
