...
def test_openat_create_methodic_1(t: LinuxTestSpec, access_mode):
    ...
    with t.make_program_and_run(ima_user, ima_user, ima_evm_dir=ima_evm_dir, umask=0) as child:
        # should pass
        dir_fd = child.open(f'/{ima_evm_dir}/dir', O_DIRECTORY, 0)
        child.openat(dir_fd, 'new_file', flags, 0o666, fatal=True)

        # AT_FDCWD = -100
        
        # # Создаем файл через openat с абсолютным путем
        # # Это должно сгенерировать событие openat_create
        # child.openat(AT_FDCWD, f'/{ima_evm_dir}/dir/new_file', flags, 0o666, fatal=True)
